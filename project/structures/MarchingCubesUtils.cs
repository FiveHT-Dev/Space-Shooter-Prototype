...
{
    public class MCCornerPoint
	{
		public Vector3 Position {get{return _position;}}
		public float Value {get{return _value;}}
        public MCVoxel OwnerVoxel {get{return _ownerVoxel;}}

		private Vector3 _position;
		private float _value;
        private MCVoxel _ownerVoxel;

		public MCCornerPoint(Vector3 position, MCVoxel ownerVoxel, MCValueOperationStack valueOperationStack)
		{
			_position = position;
            _ownerVoxel = ownerVoxel;
			_value = valueOperationStack.GetValue(this);
            //if(GD.RandRange(0, 200) == 100) GD.Print(_value);
		}
	}

    /*public class MCNoiseSampler
	{
		private FastNoiseLite _noise;
		private Vector3 _chunkGlobalPosition;
        private TileGridChunkCreationArgs _chunkCreationArgs;
		public MCNoiseSampler(FastNoiseLite noise, TileGridChunkCreationArgs chunkCreationArgs, Vector3 chunkGlobalPosition)
		{
			_noise = noise;
			_chunkGlobalPosition = chunkGlobalPosition;
            _chunkCreationArgs = chunkCreationArgs;
		}

		public float GetCornerPointValue(Vector3 pointPosition)
		{
			Vector3 cornerPointGlobalPosition = pointPosition + _chunkGlobalPosition;
			return _noise.GetNoise3Dv(cornerPointGlobalPosition);
		}
	}*/
	...
	{
		public int CubeIndex {get{return _cubeIndex;}}
		public Vector3 GridPosition {get{return _gridPosition;}}
		public List<MCCornerPoint> Points {get{return _points;}}
        public Dictionary<MCPlane, List<MCCornerPoint>> Planes {get{return _planes;}}

		private Vector3 _gridPosition;
		private List<MCCornerPoint> _points = new();
		private int _cubeIndex;
        private Dictionary<MCPlane, List<MCCornerPoint>> _planes = new();

		public MCVoxel(float voxelHalfSize, float surfaceThreshold, Vector3 gridPosition, MCValueOperationStack valueOperationStack)
		{
			_gridPosition = gridPosition * voxelHalfSize * 2;

			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(-voxelHalfSize, -voxelHalfSize, -voxelHalfSize), this, valueOperationStack)); // Point 0
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(voxelHalfSize, -voxelHalfSize, -voxelHalfSize), this, valueOperationStack)); // Point 1
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(-voxelHalfSize, voxelHalfSize, -voxelHalfSize), this, valueOperationStack)); // Point 2
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(voxelHalfSize, voxelHalfSize, -voxelHalfSize), this, valueOperationStack)); // Point 3

			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(-voxelHalfSize, -voxelHalfSize, voxelHalfSize), this, valueOperationStack)); // Point 4
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(voxelHalfSize, -voxelHalfSize, voxelHalfSize), this, valueOperationStack)); // Point 5
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(-voxelHalfSize, voxelHalfSize, voxelHalfSize), this, valueOperationStack)); // Point 6
			Points.Add(new MCCornerPoint(_gridPosition + new Vector3(voxelHalfSize, voxelHalfSize, voxelHalfSize), this, valueOperationStack)); // Point 7

            _planes[MCPlane.PosX] = [Points[1], Points[3], Points[5], Points[7]];
            _planes[MCPlane.NegX] = [Points[0], Points[2], Points[4], Points[6]];

            _planes[MCPlane.PosY] = [Points[2], Points[3], Points[6], Points[7]];
            _planes[MCPlane.NegY] = [Points[0], Points[1], Points[4], Points[5]];

            _planes[MCPlane.PosZ] = [Points[4], Points[5], Points[6], Points[7]];
            _planes[MCPlane.NegZ] = [Points[0], Points[1], Points[2], Points[3]];

			_cubeIndex = MarchingCubesUtils.GetCubeIndex(_points, surfaceThreshold);
		}
	}
	...
	public class Surface
	{
		...
		public ArrayMesh GetAsArrayMesh()
		{
    ...
    {
        public static int GetCubeIndex(List<MCCornerPoint> points, float surfaceThreshold)
		{
			int cubeIndex = 0;
			for(int i = 0; i < 8; i++)
			{
				if(points[i].Value < surfaceThreshold) cubeIndex |= 1 << i;
			}

			return cubeIndex;
		}

		public static Vector3 CalculateTriangleNormal(List<Vector3> tri)
		{
			Vector3 a = tri[2] - tri[0];
			Vector3 b = tri[1] - tri[0];
    ...
    public struct EdgeVertexIndexPair
    {
        public int Vi0 {get; private set;}
        public int Vi1 {get; private set;}

        public EdgeVertexIndexPair(int vi0, int vi1)
        {
            Vi0 = vi0;
            Vi1 = vi1;
        }
    ...
    public class MCTables
    {
        ...
        public static List<EdgeVertexIndexPair> s_EdgeVertexIndices = [
            new EdgeVertexIndexPair(0, 1),
            new EdgeVertexIndexPair(1, 3),
            new EdgeVertexIndexPair(3, 2),
            new EdgeVertexIndexPair(2, 0),
            new EdgeVertexIndexPair(4, 5),
            new EdgeVertexIndexPair(5, 7),
            new EdgeVertexIndexPair(7, 6),
            new EdgeVertexIndexPair(6, 4),
            new EdgeVertexIndexPair(0, 4),
            new EdgeVertexIndexPair(1, 5),
            new EdgeVertexIndexPair(3, 7),
            new EdgeVertexIndexPair(2, 6)
        ];

        /*public static List<int> s_EdgeTable =[
        ...
        public static List<List<int>> s_TriangleTable = [
            [ -1 ],
            [ 0, 3, 8, -1 ],
            [ 0, 9, 1, -1 ],
            [ 3, 8, 1, 1, 8, 9, -1 ],
